...
type ApiState = { mode?: "demo" | "live"; overall_health: string; collector_online: boolean; collector_last_success_at?:string|null; collector_error?:CollectorError|null; generated_at?: string; readings: Record<string, ApiReading> };
...
function useTelemetry() {
  ...
  useEffect(()=>{
    ...
    fetch(`${base}/api/state`,{signal:controller.signal}).then(r=>{if(!r.ok)throw new Error("API unavailable");return r.json() as Promise<ApiState>}).then(d=>{setState(d);setConnection(d.mode==="demo"?"demo":d.collector_online?"online":"offline")}).catch(()=>setConnection("offline"));
    ...
    stream.addEventListener("state",event=>{const payload=JSON.parse((event as MessageEvent).data) as ApiState|{state:ApiState};const next="state" in payload?payload.state:payload;setState(next);setConnection(next.mode==="demo"?"demo":next.collector_online?"online":"offline")});
    ...
    stream.onerror=()=>setConnection(c=>c==="demo"?"demo":"offline");
...
function HomeView({scenario,connection,collectorError,value,alerts,profile,go}:{scenario:Scenario;connection:Connection;collectorError?:CollectorError|null;value:(path:string)=>boolean|number|string|undefined;alerts:ApiAlert[];profile:DashboardProfile;go:(view:View)=>void}){
  const fault=scenario==="shore-loss", stale=scenario==="stale"||connection==="offline";
...
export default function Dashboard(){
  ...
  return <main className={`dashboard-shell scenario-${scenario}`}><header className="topbar"><button className="brand-lockup" type="button" onClick={()=>setView("home")} aria-label="Go to home dashboard"><span className="brand-mark">{telemetry.profile.vehicle.monogram}</span><span><strong>{telemetry.profile.vehicle.name}</strong><small>{telemetry.profile.vehicle.subtitle}</small></span></button><div className="topbar-right"><label className="scenario-control"><span>Preview state</span><select value={scenario} onChange={event=>setScenario(event.target.value as Scenario)} aria-label="Preview dashboard state"><option value="normal">Normal</option><option value="shore-loss">Shore power lost</option><option value="stale">Stale data</option></select></label><StatusPill tone={scenario==="stale"||telemetry.connection==="offline"?"off":attention?"warn":"ok"} label={scenario==="stale"?"Stale":attention?"Attention":telemetry.connection==="online"?"Live":telemetry.connection==="offline"?"Offline":"Demo"}/><span className="clock">{clockLabel}</span></div></header>
    ...
    <section className="content" aria-live="polite">{view==="home"&&<HomeView scenario={scenario} connection={telemetry.connection} collectorError={telemetry.state?.collector_error} value={telemetry.value} alerts={telemetry.alerts} profile={telemetry.profile} go={setView}/>} {view==="battery"&&<BatteryView {...detailProps}/>} {view==="ac-power"&&<AcPowerView {...detailProps}/>} {view==="climate"&&<ClimateView {...detailProps}/>} {view==="tanks"&&<TanksView {...detailProps}/>} {view==="events"&&<EventsView events={telemetry.events} alerts={telemetry.alerts} connection={telemetry.connection} acknowledgementEnabled={telemetry.profile.capabilities?.alert_acknowledgement===true&&telemetry.connection==="online"} onAcknowledge={telemetry.acknowledgeAlert}/>}<footer><span>Dashboard visualizes current state</span><b>RV Whisper alerts operate independently</b></footer></section>
  </main>;
}
