...
The software foundation and hardware-independent MVP are ready. Direct-LAN RVM3 telemetry, authenticated active-alert mirroring, and the opt-in one-alert acknowledgement flow have been validated against observed firmware behavior. A live interrupted-response test also confirmed that the dashboard fails closed, sends no automatic retry, and reconciles from the next authoritative RVM3 read. Each installation still requires its own private sensor map and dashboard profile; unsupported or uninstalled battery, tank, and other hardware remains **Not reported** rather than being estimated. Alert acknowledgement remains disabled by default and should be enabled only on a trusted local installation. See the open-question drafts before making additional hardware-specific assumptions.
